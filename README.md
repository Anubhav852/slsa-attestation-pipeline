...
4. **Immutable Runtime Environment:** The root filesystem is mounted as read-only (`readOnlyRootFilesystem: true`), preventing post-exploitation payloads or unauthorized runtime mutations.
...
| **Privileges** | `allowPrivilegeEscalation: false` | Disallows child processes from gaining more rights than parents |
...
### Prerequisites
* Docker Desktop (with Kubernetes or `kind` cluster active)
* `kubectl` CLI client
